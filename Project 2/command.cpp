...
command::command(int type, vector<string> argv, bool before, bool after, bool numbered_pipe_before, bool numbered_pipe_after, bool pipe_stderr) {
    ...
    this->in_id = 0;
    this->out_id = 0;
    this->user_pipe_in = false;
    this->user_pipe_out = false;
}
...
command::command(int type, vector<string> argv, unsigned int pipe_number, bool before, bool after, bool numbered_pipe_before, bool numbered_pipe_after, bool pipe_stderr) {
    this->type = type;
    this->argv_length = argv.size() + 1;
    this->argv = new char*[this->argv_length];
    for (int i = 0; i < argv.size(); ++i) {
