...
public:
    ...
    void start() {
        bool permit = Request_and_Read_Reply();
        if (permit) {
            open_file();
            do_read();
        } else {
...
private:
    bool Request_and_Read_Reply() {
        uint8_t request[9];
		memset(request, 0, sizeof(request));
		request[0] = 4;
		request[1] = 1;
		request[2] = client_info_.port / 256;
		request[3] = client_info_.port % 256;
		tcp::resolver resolver_(io_context);
		tcp::resolver::results_type results = resolver_.resolve(client_info_.host_name, to_string(client_info_.port), error);
		for (auto it = results.begin(); it != results.end(); it++) {
			array<unsigned char, 4> ip = it->endpoint().address().to_v4().to_bytes();
			for (int i = 0; i < 4; i++) {
				request[4+i] = int(ip[i]);
			}	
		}
		socket_->write_some(boost::asio::buffer(request, 9));
		socket_->read_some(boost::asio::buffer(read_buffer, BUFFER_MAX_LEN));
		int stat = int(read_buffer[1]);
		if (stat == 91) {
...
socks4_info get_socks4_info(string query) {
    socks4_info socks4_info_;
    size_t idx1 = query.find("&sh");
    size_t idx2 = query.find("&sp");
    socks4_info_.host_name = query.substr(idx1 + 4, idx2 - idx1 - 4);
    socks4_info_.port = stoi(query.substr(idx2 + 4, query.size() - idx2 - 4));
    return socks4_info_;
}
...
int main(int argc, char* argv[])
{
    try {
        ...
        socks4_info socks4_info_ = get_socks4_info(query);
        boost::asio::io_context io_context;
        tcp::resolver resolver_(io_context);
        cout << "Content-Type: text/html\r\n\r\n";
        print_body(client_infos);
        string sh = socks4_info_.host_name;
        string sp = to_string(socks4_info_.port);
        tcp::resolver::results_type results = resolver_.resolve(sh, sp, error);
        for (auto it = results.begin(); it != results.end(); it++) {
        ...
        for (size_t i = 0; i < 5; i++) {
            ...
            tcp::endpoint endpoint_(boost::asio::ip::address::from_string(sh), stoi(sp));
